...
def preprocess_input(input_dict):
    ...
    # ⭐ Added missing column here (MAIN BUG FIX)
    df_columns = [
        'age','region','number_of_dependants','bmi_category','income_lakhs',
        'medical_history','insurance_plan','genetical_risk','gender_Male',
        'smoking_status_Occasional','smoking_status_Regular','smoking_status_Smoking=0',
    ...
    insurance_plan_encoded = {'Silver':1,'Bronze':0,'Gold':2}

    df['region'] = region_encoded.get(input_dict['region'],1)
    df['bmi_category'] = bmi_category_encoded.get(input_dict['bmi_category'],1)
    df['insurance_plan'] = insurance_plan_encoded.get(input_dict['insurance_plan'],1)

    # One-hot
    if input_dict['gender'] == 'Male':
        df['gender_Male'] = 1

    # ⭐ MAIN FIX — smoking mapping
    smoke = input_dict['smoking_status']
...
def predict(input_dict):
    ...
    if input_dict['age'] < 25:
        pred = model_young.predict(processed_data)
    else:
        pred = model_rest.predict(processed_data)
